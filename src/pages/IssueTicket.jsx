...
export default function IssueTicket(){
  ...
  const lookup=async()=>{
    ...
    try{
      ...
      setHistory({vehicle,violations:items});
  ...
  const priorCount=history?.violations?.length||0;
  const repeatDetected=priorCount>0;
  ...
  return <div className="issue-ticket-restored">
      ...
      <div className="card-body">
        <form onSubmit={submit} className="legacy-ticket-form">
          ...
          <div className="form-grid">
            <label className="field"><span>Plate Number *</span><div className="input-action"><input required maxLength="20" placeholder="e.g., ABC1234" value={form.plate_number} onBlur={lookup} onChange={event=>setForm({...form,plate_number:event.target.value.toUpperCase()})}/><button type="button" className="btn btn-secondary btn-sm" onClick={lookup}>Lookup</button></div><small className="field-hint">Use the vehicle registration plate. Lookup fills known vehicle details and prior history.</small></label>
          ...
          {repeatDetected&&<div className="repeat-offender-alert"><div className="repeat-alert-icon">!</div><div><strong>REPEAT OFFENDER DETECTED</strong><p>This violator has <b>{priorCount}</b> prior violation(s) on record. The current offense level and penalty are calculated from the active rules.</p><StatusBadge value="repeat offender"/></div></div>}
          <div className="violation-penalty-grid">
            ...
            <div className="penalty-display"><span>Penalty Amount</span><strong>{money(displayedPenalty)}</strong>{preview&&<small>Offense #{preview.nextOffenseCount||priorCount+1}{preview.usedEscalationRule?' · escalated rule':' · base rule'}</small>}</div>
          </div>
    ...
    <Modal open={reviewOpen} title="Review Violation Ticket" onClose={()=>!busy&&setReviewOpen(false)} footer={<><button type="button" className="btn btn-secondary" disabled={busy} onClick={()=>setReviewOpen(false)}>Back to Form</button><button type="button" className="btn btn-primary" disabled={busy} onClick={confirmSubmit}><Icon name="ticket"/>{busy?'Issuing…':'Confirm and Issue Ticket'}</button></>}>
      ...
      <div className="ticket-review-grid">
        ...
        <div><small>Penalty</small><strong>{money(displayedPenalty)}</strong></div>
