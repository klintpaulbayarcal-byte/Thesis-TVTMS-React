...
export default function AdminDashboard(){
 ...
 const riskRows=useMemo(()=>{
   ...
   const rows=source.map((r,i)=>({name:r.barangay||r.location||`Area ${i+1}`,count:Number(r.total_tickets??r.total_violations??r.count??0)})).slice(0,5);
 ...
 return <div className="admin-dashboard-restored">
   ...
   <section className="map-hero"><div className="map-head"><div><p className="section-kicker">Operations View</p><h3>Recorded Citations by Area</h3></div><Link to="/officer/lookup" className="btn btn-primary btn-sm"><Icon name="search"/> Live Plate Lookup</Link></div><div className="map-canvas"><div className="map-grid-overlay"/><div className="skyline-axis-labels"><span>High</span><span>Medium</span><span>Low</span></div><div className="risk-skyline" aria-label="Recorded tickets by area">{!riskRows.length&&<p className="empty-state">No location data available. Recorded citation counts will appear here when the data is available.</p>}{riskRows.map((r,i)=>{const pct=Math.max(18,Math.round((r.count/maxRisk)*100));const level=pct>=70?'high':pct>=42?'medium':'low';return <div key={`${r.name}-${i}`} className={`risk-column risk-${level}`} style={{height:`${pct}%`}}><em>{level}</em><strong className="risk-value">{r.count}</strong><small>{r.name}</small></div>})}</div><div className="intervention-ribbon"><span>{riskRows[0]?.count>0?`${riskRows[0].name} currently has the highest recorded activity in this dashboard view.`:'Risk levels will populate from live location data as ticket records are available.'}</span></div></div><div className="map-meta-row"><div className="map-meta"><span className="dot high"/>Higher recorded count</div><div className="map-meta"><span className="dot medium"/>Moderate recorded count</div><div className="map-meta"><span className="dot low"/>Lower recorded count</div></div></section>
