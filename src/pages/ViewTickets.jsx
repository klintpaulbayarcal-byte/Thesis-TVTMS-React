...
export default function ViewTickets(){
  ...
  const exportCsv=()=>{const data=[['Ticket','Date','Plate','Owner','Violation','Penalty','Status'],...rows.map(row=>[row.ticket_number,dateOnly(row.date_issued),row.plate_number,row.owner_name,row.violation_name,row.penalty_amount,row.status])];const csv=data.map(line=>line.map(csvCell).join(',')).join('\n');const url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='tvtms-tickets.csv';a.click();URL.revokeObjectURL(url);};
