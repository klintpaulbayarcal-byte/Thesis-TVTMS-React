...
{
    $plate=normalize_plate($_GET['plate_number']??$_GET['plateNumber']??$_GET['plate']??'');if(!$plate)fail('Plate number is required.',400,'VALIDATION_ERROR');$r=supabase_rpc('tvtms_public_summary',['p_plate'=>$plate]);$r=is_array($r)?$r:[];$total=(int)($r['total_violations']??0);json_response(['success'=>true,'plate_number'=>$plate,'summary'=>['total_violations'=>$total,'unpaid_count'=>(int)($r['unpaid_count']??0),'paid_count'=>(int)($r['paid_count']??0),'cancelled_count'=>(int)($r['cancelled_count']??0),'total_unpaid_amount'=>(float)($r['total_unpaid_amount']??0),'total_demerit_points'=>(int)($r['total_demerit_points']??0),'is_repeat_offender'=>$total>=2]]);
}
