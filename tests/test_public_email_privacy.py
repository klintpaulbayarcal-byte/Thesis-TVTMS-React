...
def test_public_lookup_maps_masked_email_by_ticket_id_without_private_fields():
...
function supabase_rpc(string $name,array $args=[]): mixed {{ return [
  ["id"=>22,"ticket_number"=>"TVT-2026-000022","status"=>"unpaid","payment_status"=>"unpaid","owner_email"=>"leaked@example.test","dispute_deadline_days"=>15,"dispute_age_days"=>2,"has_open_dispute"=>0],
  ["id"=>11,"ticket_number"=>"TVT-2026-000011","status"=>"unpaid","payment_status"=>"unpaid","dispute_deadline_days"=>15,"dispute_age_days"=>3,"has_open_dispute"=>0]
]; }}
...
public_ticket_lookup();'''
    ...
    assert output["selectCall"][0] == "ticket_details"
    assert output["selectCall"][2]["select"] == "id,owner_email"
...
def test_public_lookup_ui_uses_only_the_dedicated_masked_email_fields():
    source = (ROOT / "src/pages/PublicTicketLookup.jsx").read_text(encoding="utf-8")
    assert "Notification Email" in source
    assert "ticket.has_notification_email" in source
    assert "ticket.notification_email_masked" in source
    assert "No email recorded" in source
